...
class ProductsController extends Controller
{
	...
	public function purchase(Request $request, Product $product)
	{
		...
		if ($product->stock <= 0) {
			return redirect()->route('products_list')->withErrors('Product is out of stock.');
		}

		// Check if the user has sufficient credit
		if ($user->credit < $product->price) {
			return redirect()->route('products_list')->withErrors('Insufficient credit to purchase this product.');
		}
